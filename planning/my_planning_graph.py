...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        # TODO add action A level to the planning graph as described in the Russell-Norvig text 1. determine what
        # actions to add and create those PgNode_a objects 2. connect the nodes to the previous S literal level for
        # example, the A0 level will iterate through all possible actions for the problem and add a PgNode_a to
        # a_levels[0] set iff all prerequisite literals for the action hold in S0.  This can be accomplished by
    ...
    def add_literal_level(self, level):
        ...
        """
        # TODO add literal S level to the planning graph as described in the Russell-Norvig text 1. determine what
        # literals to add 2. connect the nodes for example, every A node in the previous level has a list of S nodes
        # in effnodes that represent the effect produced by the action.  These literals will all be part of the new S
        # level.  Since we are working with sets, they may be "added" to the set without fear of duplication.
        # However, it is important to then correctly create and connect all of the new S nodes as children of all the
        # A nodes that could produce them, and likewise add the A nodes to the parent sets of the S nodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        node1 = len(set(node_a1.action.effect_add) & set(node_a2.action.effect_rem))
        node2 = len(set(node_a2.action.effect_add) & set(node_a1.action.effect_rem))
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        node1_add = len(set(node_a1.action.effect_add) & set(node_a2.action.precond_neg))
        node1_rem = len(set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos))
        node2_add = len(set(node_a2.action.effect_add) & set(node_a1.action.precond_neg))
        node2_rem = len(set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos))

        if node1_add > 0 or node1_rem > 0 or node2_add > 0 or node2_rem > 0:
            return True
